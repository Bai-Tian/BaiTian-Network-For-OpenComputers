...
modem.send(BaitianDNS.control,23,"cmdline","info","DNS server <"..BaitianDNS.servername.."> is starting...")
...
local function dns_r()
    if BaitianDNS.domain[BaitianDNS.domainpre] and BaitianDNS.domain[BaitianDNS.domainpre]==BaitianDNS.from then
        ...
        modem.send(BaitianDNS.control,23,"cmdline","info","#"..string.format("%3.3s",BaitianDNS.from).." edited <"..BaitianDNS.domainpre.."."..BaitianDNS.servername..">")
        modem.send(BaitianDNS.from,53,"edited")
        BaitianLib.configrewrite()
    elseif not BaitianDNS.domain[BaitianDNS.domainpre] then
        BaitianDNS.domain[BaitianDNS.domainpre] =BaitianDNS.from
        modem.send(BaitianDNS.control,23,"cmdline","info","#"..string.format("%3.3s",BaitianDNS.from).." registered <"..BaitianDNS.domainpre.."."..BaitianDNS.servername..">")
        modem.send(BaitianDNS.from,53,"created")
        BaitianLib.configrewrite()
    else
        modem.send(BaitianDNS.control,23,"cmdline","info","#"..string.format("%3.3s",BaitianDNS.from).." tried to register <"..BaitianDNS.domainpre.."."..BaitianDNS.servername..">,but it failed")
        modem.send(BaitianDNS.from,53,"failed")
    end
end

local function dns_l()
	modem.send(BaitianDNS.control,23,"cmdline","info",BaitianLib.read("/"..component.computer.address .. "-DOMAIN.lua"))
end

local function dns_sa()
	if BaitianDNS.swm then
		modem.setWakeMessage(BaitianDNS.swm)
		modem.send(BaitianDNS.control,23,"cmdline","info","setwakemessage")
	else
		modem.send(BaitianDNS.control,23,"cmdline","warn","setwakemessage failed")
	end
end

local function dns_st()
	modem.send(BaitianDNS.control,23,"cmdline","info","shutdown")
	computer.shutdown()
end
...
modem.send(BaitianDNS.control,23,"cmdline","info","DNS service has been started successfully!")